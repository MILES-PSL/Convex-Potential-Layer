...
def dont_normalise(x, mean=cifar10_mean, std=cifar10_std):
...
class Crop(namedtuple('Crop', ('h', 'w'))):
    ...
    def undo(self, x, x0, y0): 
        C, H, W = self.x_shape
        return np.pad(x, [(0,0), (y0,H-(y0+self.h)), (x0,W-(x0+self.w))],
...
class FlipLR(namedtuple('FlipLR', ())):
    ...
    def undo(self, x, choice): 
        return self(x,choice)
