...
# def warmup_cudnn(model, batch_size,num_classes=100):
...
#         'target': torch.LongTensor(np.random.randint(0, num_classes, batch_size)).cuda()
#     }
#     model.train(True)
#     o = model(batch)
#     o['loss'].sum().backward()
...
class Batches():
    def __init__(self, dataset, batch_size, shuffle, set_random_choices=False,
                 num_workers=0, drop_last=False, return_perturbation=False):
